...
<%@ taglib prefix="common" uri="http://hime.cn/common/"%>
<%
...
<body>
		...
		<div id="page-wrapper">
			...
			<div class="panel panel-default">
				<div class="panel-body">
					<form class="form-inline" action="${pageContext.request.contextPath }/index.action" method="post">
						<div class="form-group">
							<label for="customerName">客户名称</label>
							<input type="text" class="form-control" id="customerName" value="${param.custName }" name="custName" />
						</div>
						<div class="form-group">
							<label for="customerFrom">客户来源</label>
							<select	class="form-control" id="customerFrom" name="custSource">
								<option value="">--请选择--</option>
								<c:forEach items="${requestScope.sourceType}" var="item">
									<option value="${item.dict_id}" <c:if test="${item.dict_id == param.custSource}"> selected</c:if> >${item.dict_item_name }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custIndustry"  name="custIndustry">
								...
								<c:forEach items="${industryType}" var="item">
									<option value="${item.dict_id}"<c:if test="${item.dict_id == param.custIndustry}"> selected</c:if>>${item.dict_item_name }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custLevel" name="custLevel">
								...
								<c:forEach items="${levelType}" var="item">
									<option value="${item.dict_id}"<c:if test="${item.dict_id == param.custLevel}"> selected</c:if>>${item.dict_item_name }</option>
								</c:forEach>
							</select>
						</div>
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
						...
						<div class="col-md-12 text-right">
							<common:page url="${pageContext.request.contextPath }/index.action" />
						</div>
						<!-- /.panel-body -->
					</div>
					<!-- /.panel -->
	...
	<script type="text/javascript">
		...
		function updateCustomer() {
			...
			//alert(typeof $("#edit_customer_form").serialize());
			$.post(
